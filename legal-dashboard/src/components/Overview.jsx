...
export default function Overview({ stats }) {
  ...
  const loadDetailedStats = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5001/api/stats/detailed')
  ...
  const loadTokenStats = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5001/api/stats/tokens')
      const data = await response.json()
      if (data.success) {
        ...
        <Grid container spacing={{ xs: 2, sm: 2.5, md: 3 }}>
          ...
          <Grid item xs={12} md={6}>
            <Card sx={{ border: '1px solid rgba(33, 150, 243, 0.3)' }}>
              <CardContent>
                <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>
                  Token Insights & Cost Estimates
                </Typography>
                {tokenStats ? (
                  <Fade in={true} timeout={1200}>
                    <Box sx={{ pl: 2 }}>
                      <Typography variant="body1" sx={{ mb: 2, color: '#b3e5fc' }}>
                        <strong>Total Tokens:</strong> {tokenStats.total_tokens?.toLocaleString()}
                      </Typography>
                      <Typography variant="body1" sx={{ mb: 2, color: '#b3e5fc' }}>
                        <strong>Avg Tokens/Sample:</strong> {tokenStats.avg_tokens_per_sample?.toLocaleString()}
                      </Typography>
                      <Typography variant="body1" sx={{ mb: 2, color: '#b3e5fc' }}>
                        <strong>Encoding:</strong> {tokenStats.encoding}
                      </Typography>
                      <Typography variant="subtitle2" sx={{ mt: 3, mb: 2, color: '#2196f3', fontWeight: 600 }}>
                        Training Cost Estimates:
                      </Typography>
                      {tokenStats.estimated_costs && Object.entries(tokenStats.estimated_costs).slice(0, 4).map(([model, cost]) => (
                        <Typography key={model} variant="body2" sx={{ mb: 1, color: '#b3e5fc', fontSize: '0.85rem' }}>
                          <strong>{cost.name}:</strong> ${cost.cost_usd.toFixed(4)}
                        </Typography>
                      ))}
                    </Box>
                  </Fade>
                  ...
                  <Box sx={{ pl: 2 }}>
                    <Skeleton variant="text" width="80%" height={30} sx={{ mb: 2, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="70%" height={30} sx={{ mb: 2, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="60%" height={30} sx={{ mb: 3, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="90%" height={25} sx={{ mb: 2, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="85%" height={20} sx={{ mb: 1, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="85%" height={20} sx={{ mb: 1, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="85%" height={20} sx={{ mb: 1, bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                    <Skeleton variant="text" width="85%" height={20} sx={{ bgcolor: 'rgba(33, 150, 243, 0.1)' }} />
                  </Box>
                )}
              </CardContent>
            </Card>
